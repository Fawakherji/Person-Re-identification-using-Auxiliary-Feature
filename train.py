...
parser.add_argument('--num-epoch', default=200, type=int, help='num of epoch')
parser.add_argument('--num-workers', default=2, type=int, help='num_workers')
parser.add_argument('--use_id', action='store_true', help='use identity loss')
parser.add_argument('--lamba', default=0.9, type=float, help='weight of id loss')
...
def plot_graph(name,train_loss,valid_loss):
	fig = plt.figure(figsize=(10,8))
	plt.plot(range(1,len(train_loss)+1),train_loss, label='Training '+name)
	plt.plot(range(1,len(valid_loss)+1),valid_loss,label='Validation '+name)
	plt.xlabel('epochs')
	plt.ylabel(name)
	fig.savefig('loss_plo_t___1'+name+'.png', bbox_inches='tight')
#plt.ylim(0, 0.5) # consistent scale
#plt.xlim(0, len(train_loss)+1) # consistent scale
	fig.savefig('loss_plo_t___'+name+'.png', bbox_inches='tight')
#plt.grid(True)
	plt.legend()
	fig.savefig('loss_plo_t__'+name+'.png', bbox_inches='tight')
	plt.tight_layout()
	fig.savefig('loss_plo_t'+name+'.png', bbox_inches='tight')
...
#plt.xlim(0, len(train_loss)+1) # consistent scale
	...
	plt.tight_layout()
#plt.show()
	fig.savefig('loss_plot'+name+'.png', bbox_inches='tight')
...
#out = model(x)
#make_dot(out).render("reid_after_mode", format="png")
...
def train_model(model, optimizer, scheduler, num_epochs):
    ...
    for epoch in range(1, num_epochs+1):
        ...
        for phase in ['train', 'val']:
            ...
            if phase == 'train':
                avg_train_acc.append(epoch_acc)
                avg_train_losses.append(epoch_loss)
            if phase == 'val':
                avg_valid_acc.append(epoch_acc)
    ...
    plot_graph('loss',avg_train_losses,avg_valid_losses)
    plot_graph('acc',avg_train_acc,avg_valid_acc)
