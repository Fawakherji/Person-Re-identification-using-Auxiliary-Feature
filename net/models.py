...
from net.utils import ClassBlock,ClassBlock2
...
class Backbone_nFC_Id(nn.Module):
    def __init__(self, class_num, id_num, model_name='resnet50_nfc_id'):
        ...
        for c in range(self.class_num+1):
            if c == self.class_num:
                self.__setattr__('class_%d' % c, ClassBlock2(self.num_ftrs, class_num=self.id_num, activ='none'))
            else:
    ...
    def forward(self, x):
        ...
        pred_label = [self.__getattr__('class_%d' % c)(x) for c in range(self.class_num)]
        pred_label = torch.cat(pred_label, dim=1)
        #print ('pred_lable*****************************',pred_label.shape)
        #print ('self.features(x)*****************************+',x.shape)
        pred_id = self.__getattr__('class_%d' % self.class_num)(x,pred_label)
